...
#include <iostream>
#include <sstream>//for sstream in deserialize to read like a string 
...
string Admin::serialize() const { //convert data into long string
    string roleStr;
    switch (roleType_) {
...
Admin* Admin::deserialize(const string& line) { //returns a pointer to admin obj //function recieves one string
    stringstream ss(line); // convert string again into pieces
    string token;
    ...
    getline(ss, phone, '|');
    getline(ss, address, '|');
    getline(ss, token, '|'); balance = stod(token); //stod : string to double...
    ...
    if (!ss.eof()) {//end of file
        string tempCheck;
        size_t checkPos = ss.tellg();//ss.tell == saves current string pos   // size_t =builtin data type to stoe pos
        if (getline(ss, tempCheck, '|')) {
            ...
            if (tempCheck.find("ROLE_") != string::npos || tempCheck == "SUPER_ADMIN" || tempCheck.empty()) {
                ...
                roleStr = nextToken;
                ss.seekg(checkPos);//go back to active 
            }
        }
    } else {
        roleStr = nextToken;
