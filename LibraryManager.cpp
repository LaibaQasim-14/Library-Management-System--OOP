...
#include <fstream>//file save n load
#include <ctime>
#include <iostream>
#include <algorithm>//sort,find,count,
#include <sstream>//convert int /string
#include <vector>
#include <cctype>//charactr checking is alpha, isdigit
...
void LibraryManager::seedSampleResources()
{
    ...
    for (const auto& book : books) // auto compiler figures datatype on its own
...
{
    ifstream file(resourceFilename_); //ifstream readfile
    if (!file.is_open()) // if file missing exit safely
        return;

    for (auto* resource : resources_)
        delete resource;
    resources_.clear(); //remove all pointers from vector

    string line;
    while (getline(file, line)) { //read fiile line by line
        if (line.empty()) // ignore blank lines
            continue;

        try {
            stringstream ss(line); //converts into stream
            string idToken;
            string typeToken;
            getline(ss, idToken, '|');
